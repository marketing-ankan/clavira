...
export default function Dashboard() {
    ...
    if (!stats) return <p className="text-charcoal/40 font-display text-lg">Loading…</p>;
    ...
    return (
        <div className="space-y-6">
                ...
                <Card title="Today's gold rate (per gram)">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        {[['24kt', stats.gold_rate.rate_24k], ['22kt', stats.gold_rate.rate_22k], ['18kt', stats.gold_rate.rate_18k], ['14kt', stats.gold_rate.rate_14k]].map(([k, v]) => (
                            <div key={k}>
                                <span className="text-charcoal/50 text-[11px] uppercase tracking-[0.14em]">{k}</span>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                <Card title="Recent orders" action={<Link to="/admin/orders" className="text-xs text-gold hover:underline">View all</Link>}>
                    <table className="w-full text-sm">
                        <tbody>
                            ...
                            {stats.recent_orders.length === 0 && <tr><td className="py-6 text-center text-charcoal/40">No orders yet.</td></tr>}
                        </tbody>
                    </table>
                </Card>

                <Card title="Recent enquiries" action={<Link to="/admin/enquiries" className="text-xs text-gold hover:underline">View all</Link>}>
                    <div className="space-y-3">
                        {stats.recent_enquiries.map((e) => (
                            <div key={e.id} className="border-b border-gold/10 last:border-0 pb-3 last:pb-0">
                                <div className="flex justify-between gap-3">
                                    <p className="text-sm font-medium">{e.name} <span className="text-charcoal/40 font-normal">· {e.email}</span></p>
                                    <StatusBadge status={e.status} />
                                </div>
                                <p className="text-xs text-charcoal/60 mt-1 line-clamp-2">{e.message}</p>
                            </div>
                        ))}
                        {stats.recent_enquiries.length === 0 && <p className="py-6 text-center text-charcoal/40 text-sm">No enquiries yet.</p>}
                    </div>
                </Card>
            </div>
        </div>
