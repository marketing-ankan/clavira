        ...
        <div className="space-y-8">
            ...
            <section>
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Link to="/account/orders" className="text-xs text-gold hover:underline">View all</Link>
                </div>
                {orders?.data?.length ? (
                    <div className="border border-gold/20 divide-y divide-gold/10">
                        {orders.data.slice(0, 4).map((o) => (
                            <Link key={o.id} to={`/account/orders/${o.order_no}`} className="flex items-center justify-between px-4 py-3 hover:bg-gold/5">
                                <div>
                                    <p className="text-sm font-medium text-gold">{o.order_no}</p>
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] uppercase tracking-wider text-charcoal/50">{o.status}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
                ) : (
                    <p className="text-charcoal/50 text-sm border border-gold/20 px-4 py-8 text-center">
